...
class DeviceActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<Button>(R.id.bt_send).setOnClickListener {
            ...
            if (hexSendCheckBox?.isChecked == true) {
                ...
                if (!Pattern.compile("^[0-9a-fA-F]+$").matcher(sendDataEditText?.text.toString())
                    ...
                    showAlert("Alert", "Character can only be 0-9、a-f、A-F") {}
                    return@setOnClickListener
                }
        ...
        ECBLE.onBLECharacteristicValueChange { hex, string ->
            runOnUiThread {
                val timeStr = SimpleDateFormat("[HH:mm:ss,SSS]:").format(Date(System.currentTimeMillis()))
                val nowStr = receiveDataTextView?.text.toString()
                Log.e("tag", string[0].toInt().toString())
                if (hexRevCheckBox?.isChecked == true) {
                    receiveDataTextView?.text = nowStr + timeStr + hex +"kg" + "\r\n"
                } else {
                    receiveDataTextView?.text = nowStr + timeStr + string[0].toInt().toString() + "kg"+ "\r\n"
                }
